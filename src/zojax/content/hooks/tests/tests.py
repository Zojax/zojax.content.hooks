...
from zojax.content.type.interfaces import IItem, ISearchableContent
...
class ITestHook(interface.Interface):
    """ """
...
class TestHook(object):
    interface.implements(IContentHook)
...
contentHooksLayer = ZCMLLayer(
    os.path.join(os.path.split(__file__)[0], 'ftesting.zcml'),
    __name__, 'contentHooksLayer', allow_teardown=True)
...
def test_suite():
    return unittest.TestSuite((
        FunctionalDocFileSuite(
            'tests.txt', layer=contentHooksLayer),))
